...
<center>
	<form action="subscribe" commandName="subscription">
	<table>
		<tr>
		...
		<td><select name="channel_id">
			...
			<c:forEach var="element" items="${lists}">
				<option value="${element.channel_id}"><c:out value="${element.channel_name}"/> </option>
			</c:forEach>
		</select></td>
		</tr>
		<tr><td>Date Of Subscription</td><td><input type"date" name="subsc_date" value="<%= new SimpleDateFormat("MM-dd-yyyy").format(date) %>"> </td></tr>
